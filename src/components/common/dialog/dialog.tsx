...
import { useDialogStore } from "@/state/stores/stocks-store";
...
export const Dialog = React.memo(({ title }: DialogProps) => {
  const { isOpenChange } = useDialogStore();
  return (
    <DialogContent className="w-4/5 rounded-xl bg-white">
      ...
      <DialogFooter>
        <Button className="mt-5 rounded-lg bg-main-700" onClick={() => isOpenChange(false)}>
